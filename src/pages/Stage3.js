...
function Stage3() {
        ...
        const handleSubmit = async(e)=>{
            e.preventDefault()
            try {
              const res = await axios.post("http://localhost:8080/api/v1/auth/login",{companyName,CompanyWebsite,position,companySize})
              if (res.data.success) {
...
<div style={{textAlign:'left'}} className='heading16px'>
  <div className="form-check form-check-inline" onClick={handleIndividual}>
    <input className="form-check-input" type="radio" name="inlineRadioOptions" id="inlineRadio1" defaultValue="option1" checked/>
...
<div className="row">
                    <div className="col-md-6">
                      <input type="text" className="form-control" value={companyName} id="exampleInputPassword1" placeholder='Company Name' onChange={(e) => setCompanyName(e.target.value)} />
                    </div>
                    <div className="col-md-6">
                    ...
                    <div className="col-md-6">
                      <input type="text" className="form-control" value={position} id="exampleInputEmail1" aria-describedby="emailHelp" placeholder='Role/Position' onChange={(e) => setPosition(e.target.value)} />
                    </div>
                    <div className="col-md-6">
